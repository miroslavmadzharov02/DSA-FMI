...
template<typename T>
inline Vector<T>::VectorIterator::VectorIterator(T* ptr, const size_t& index)
	: current_(ptr), index_(index)
{
}

template<typename T>
inline typename T* Vector<T>::VectorIterator::operator->()
{
...
template<typename T>
inline typename Vector<T>::VectorIterator& Vector<T>::VectorIterator::operator++()
{
	++index_;
	return *this;
}
...
template<typename T>
inline typename Vector<T>::VectorIterator& Vector<T>::VectorIterator::operator++(int)
{
...
template<typename T>
inline bool Vector<T>::VectorIterator::operator==(const VectorIterator& other) const
{
...
template<typename T>
inline typename Vector<T>::VectorIterator Vector<T>::begin()
{
	return VectorIterator(container_, 0);
}

template<typename T>
inline typename Vector<T>::VectorIterator Vector<T>::end()
{
	return VectorIterator(container_ + getSize(), getSize());
}
